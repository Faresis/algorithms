...
class QuickSort3App {
    public static void main(String[] args) {
        ...
        // inverse data
        ArrayQ inverse = new ArrayQ(maxSize);
        fillInversely(maxSize, inverse);
        System.out.println("Inverse data.");
        measure(inverse);

        // random data
        ArrayQ random = new ArrayQ(maxSize);  // create the array
        fillRandomly(maxSize, random);
        System.out.println("Random data.");
        measure(random);

        // sorted data
        ArrayQ sortedData = new ArrayQ(maxSize);
        fillSorted(maxSize, sortedData);
        System.out.println("Sorted data.");
        measure(sortedData);
    }  // end main()

    private static void fillInversely(int maxSize, ArrayQ inverse) {
        for (int i = maxSize; i > 0; --i) {
            inverse.insert(i);
        }
    }

    private static void fillRandomly(int maxSize, ArrayQ random) {
    ...
    private static void fillSorted(int maxSize, ArrayQ sorted) {
        for (int i = 0; i < maxSize; i++) {
            sorted.insert(i);
        }
    }

    private static void measure(ArrayQ arr) {
        long start = System.nanoTime();
        arr.quickSort();   // sort them
